...
function saveCreds() {
  sbUrl = document.getElementById('sb-url').value.trim();
  sbKey = document.getElementById('sb-key').value.trim();
  localStorage.setItem(CREDS_KEY, JSON.stringify({ url: sbUrl, key: sbKey }));
}
function loadCreds() {
  try { const c = JSON.parse(localStorage.getItem(CREDS_KEY)); if (c) { sbUrl = c.url || ''; sbKey = c.key || ''; } } catch {}
